...
//            gameView.scene.rootNode.position = position
    ...
    @objc func didTap(_ tap:UITapGestureRecognizer) {
        ...
        if results.count > 0 {
            ...
            let charWorldPos = (gameController.mainCharacter?.node?.worldPosition) ?? SCNVector3(0, 0, 0)

            let dirx = targetCoord.x - charWorldPos.x
            let diry = targetCoord.y - charWorldPos.y
            
            gameController.mainCharacter?.direction = float2(dirx, diry)
